...
vector<int> union_of_sorted_array (vector <int> &a, vector<int> &b) {
    ...
    while (i < n1 && j < n2) {
        if (a[i] <= b[j]){
            if (unionArr.size() == 0 || unionArr.back() != a[i]){    // means if the size of the array is zero or the last element of the array is not similar to the current element then push it
...
int main() {
    ...
    cout << endl <<  "Now, the union of these two sorted array is: ";
    for (auto it: ans) {
        cout << it << " ";
    } 
    return 0;
